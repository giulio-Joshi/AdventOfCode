...
fn main() {
    ...
    let file_content = fs::read_to_string("input.txt")
        ...
        let crabs = file_content.split(',')
        .map(|x| x.parse::<i32>().unwrap()) .collect::<Vec<i32>>();
    
        println!("Fuel spent to reach pos : {}", calc_best( &crabs, Method::Standard));
        println!("Fuel spent to reach pos, 2nd row : {}", calc_best( &crabs, Method::NthTriangle));
}
...
fn calc_best( crabs : &[i32] , method : Method) -> i32 {
    ...
    (min..max).into_iter()
        .map( | target_pos | {
            crabs.iter().map(| &crab_pos | fuel_calc (target_pos, crab_pos)).sum()
        }).min().unwrap()
}
...
mod test {
    ...
    fn test_fuel(){
        ...
        let positions : Vec<i32> = 
            String::from("16,1,2,0,4,2,7,1,2,14").split(',')
