...
fn main() {
    ...
    let scan_sweeps = fs::read_to_string("scan_sweeps.log")
                    .expect("Missing required file for input");


    let sweeps: Vec<i32> = scan_sweeps.lines().map(|x| x.parse::<i32>().unwrap())
                .collect();
