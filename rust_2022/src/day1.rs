...
pub fn execute(raw_input: &str) {
    ...
    println!(
        "Found the max amount of snacks: {}",
        get_top_snack_holder(&mut elven_amount, 1)
    );
    println!(
        "Found the three max amount of snacks: {}",
        get_top_snack_holder(&mut elven_amount, 3)
    );
}
...
fn calc_max_sum(elves: Vec<Vec<u32>>) -> Vec<u32> {
    elves.iter().map(|x| x.iter().sum()).collect::<Vec<u32>>()
}
