...
pub fn execute(input: &str) {
    ...
    let as_singles = input.lines().map(|x| x.to_owned()).collect::<Vec<String>>();

    println!("Found sum: {} ", sum_common_content(&all_sacks));
    println!(
        "Found common sums: {} ",
        sum_common_by_group(&as_singles, 3)
    );
}

fn sum_common_content(sacks: &[(String, String)]) -> u32 {
    let all_things = sacks
        .iter()
        .map(find_common)
        .collect::<Vec<String>>()
        .iter()
...
fn find_common_chunks(chunk: &[String]) -> String {
    let tmp_result = chunk
        .windows(2)
        .map(|w| find_common(&(w[0].clone(), w[1].clone())))
        .collect::<Vec<String>>();
...
fn find_common(sack: &(String, String)) -> String {
    ...
    content0.retain(|f| sack.1.contains(f));
...
mod test {
    ...
    pub fn sacks_split() {
        ...
        all_sacks.iter().take(1).for_each(|x| {
