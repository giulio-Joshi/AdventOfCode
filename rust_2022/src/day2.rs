...
fn strategy_two(input: &str) -> u32 {
    let battles: Vec<(Sign, Outcome)> = input
        .split('\n')
        .map(|v| {
...
fn guess_score(battles: &[(Sign, Outcome)]) -> u32 {
    battles.iter().map(|x| x.1.reverse(&x.0)).sum()
}
...
fn read_battle(input: &str) -> Vec<(Sign, Sign)> {
    input
        .split('\n')
        .map(|v| {
            let stuff: Vec<Sign> = v.split(' ').map(Sign::from).collect();
            (stuff[0], stuff[1])
        })
        .collect()
}
...
fn strategy_one(battles: &[(Sign, Sign)]) -> u32 {
    battles
        .iter()
        .fold(0, |accum, elemen| accum + single_match_score(elemen))
}
