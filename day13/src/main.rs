use std::{collections::HashSet, fs, num::ParseIntError, str::FromStr};
...
mod test {
    ...
    fn convert_test() {
        ...
        stuff.0.iter().for_each(|x| println!("{:?}", &x));
        ...
        for instr in stuff.1.into_iter().take(1) {
            ...
            temp.iter().for_each(|x| println!("{:?}", &x));
        ...
        //println!(" Final == ");
        filtered.iter().for_each(|x| println!("{:?}", &x));
