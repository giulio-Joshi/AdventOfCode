...
fn main() {
    ...
    let mut completed: Vec<i64> = file_content.lines().map(complete_lines).flatten().collect();
...
fn complete_lines(input: &str) -> Option<i64> {
    ...
    Some(
        result
            ...
            .into_iter()
            .rev()
            .fold(0, |accum, val| accum * 5 + completant_value(&val)),
    )
}
...
#[cfg(test)]
mod test {
    use crate::{complete_lines, progress_count};
