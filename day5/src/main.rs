...
fn main() {
    ...
    let all_vents: Vec<Vent> = file_content
        .lines()
        .map(|x| x.parse::<Vent>().unwrap())
        .collect();

    println!(
        "Found {} overlapping vents",
        count_double_points(&all_vents, false)
    );
    println!(
        "Found {} overlapping vents with diagonal ",
        count_double_points(&all_vents, true)
    );
}
...
impl Vent {
    pub fn get_points(&self, diagonal: bool) -> Vec<(i32, i32)> {
        ...
        if self.start.0 == self.end.0 {
            ...
            let mut tot: Vec<(i32, i32)> = (0..range_horiz)
                .zip(0..range_vert)
                .map(|p| {
                    (
                        self.start.0 - p.0 * horiz_dir,
                        self.start.1 - p.1 * vert_dir,
                    )
                })
                .collect();
...
mod test {
    ...
    use crate::{count_double_points, Vent};
